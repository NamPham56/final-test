    ...
    Axios.interceptors.response.use(
        (response) => {
            ...
            if (response.config.responseType == 'json') {
                // Spring Boot LMS uses ApiResponse { success, message, data, timestamp }.
                if (response.data && typeof response.data.success === 'boolean') {
                    if (!response.data.success) {
                        if (options.showCodeMessage) ElNotification({ type: 'error', message: response.data.message, zIndex: SYSTEM_ZINDEX })
